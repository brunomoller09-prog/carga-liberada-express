...
import { syncReleaseToSharePoint } from "@/lib/sharepoint-sync.functions";
...
function extractNfInfo(barcode: string): { nf: string; serie: string } {
  ...
  const numero = clean.substring(27, 34);
...
function LiberacaoCarga() {
  ...
  const syncToSharePoint = useServerFn(syncReleaseToSharePoint);
  ...
  async function finalizar(e: React.FormEvent) {
    ...
    // Espelha a liberação no SharePoint em segundo plano. Não usa
    // "await" de propósito — a pessoa no formulário não deve esperar
    // essa chamada extra pra ver a tela de sucesso, e se o SharePoint
    // falhar ou demorar, isso não deve travar nem mostrar erro aqui
    // (o Supabase já salvou com sucesso, que é o que importa de
    // verdade). Qualquer problema fica registrado só nos logs do
    // servidor (Render → Logs).
    syncToSharePoint({ data: payload as Record<string, string | number | null> }).catch(() => {
      // Erro já foi tratado e logado dentro da própria função de
      // servidor — aqui só evitamos um "unhandled promise rejection"
